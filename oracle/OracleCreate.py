...
from .integrityOracle12 import get_string_create_oracle_scheme, \
    check_success_result_create_oracle_scheme, \
...
# Класс отображения элементов схем данных для создания схем в СУБД
class SchemeGUISuit:
    def __init__(self,
                 window,
                 scheme_id,
        ...
        self.label_2.grid(column=5, row=start_row_position + 1, sticky='W')


# Основной класс отображения всех элементов окна для создания схем в СУБД
class OracleCreate(Frame):
    def __init__(self, parent, loop, config_file):
        ...
        self.button_create_scheme = Button(self.window,
                                           text='Создать схему',
                                           command=self.create_schemes)
        self.button_create_scheme.grid(column=0, columnspan=3, padx=PADX_LEFT_BORDER, pady=18, row=self.shift_row_position + 1, sticky='WE')
    ...
    def create_schemes(self):
        ...
        for item in self.scheme_gui_suits:
            if item.checked.get() == int(item.scheme_id):
                ...
                oracle_string, oracle_string_mask_password = get_string_create_oracle_scheme(sysdba_name,
                                                                                             sysdba_password,
                                                                                             connection_string,
                                                                                             scheme_name,
                                                                                             scheme_password)
                self.loop.create_task(self.run_async_cmd_with_check_and_run_next_functions(oracle_string,
                                                                                           ...
                                                                                           check_success_result_create_oracle_scheme,
                                                                                           run_next_function=self.grant_privilege_schemes,
                                                                                           label_item=item.label_1,
                                                                                           label_text="created schema"))
        self._save_current_config()

    def grant_privilege_schemes(self):
        connection_string = self.connection_string_gui_suit.field_connection_string.get()
        sysdba_name = self.sysdba_user_string_gui_suit.field_sysdba_name_string.get()
        sysdba_password = self.sysdba_user_string_gui_suit.field_sysdba_password_string.get()
        for item in self.scheme_gui_suits:
            if item.checked.get() == int(item.scheme_id):
                scheme_name = item.field_scheme_name.get()
                oracle_string, oracle_string_mask_password = get_string_grant_oracle_privilege(sysdba_name,
                                                                                               sysdba_password,
                                                                                               connection_string,
                                                                                               scheme_name)
                self.loop.create_task(self.run_async_cmd_with_check_and_run_next_functions(oracle_string,
    ...
    def _start_main_progressbar(self):
        ...
        self.button_create_scheme.configure(state=DISABLED)

    def _stop_main_progressbar(self):
        self.main_progressbar.stop()
        self.main_progressbar.grid_forget()
        self.button_create_scheme.configure(state=NORMAL)
