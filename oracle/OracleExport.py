...
from .integrityOracle12 import get_string_check_oracle_connection, \
    ...
    check_failure_result_export_oracle_scheme, \
...
WINDOW_TITLE = 'Oracle. Экспорт схем'
...
class SchemeGUISuit:
    def __init__(self,
        ...
        self.scheme_name_export_dump_file = StringVar(value=export_dump_file)
        ...
        self.label_scheme_export_dump_file = Label(self.window, text='Файл для экспорта', state=DISABLED)
        self.label_scheme_export_dump_file.grid(column=4, row=start_row_position + 1, padx=3, sticky='E')
        self.field_scheme_export_dump_file = Entry(self.window, textvariable=self.scheme_name_export_dump_file,
                                                   width=WIDTH_SCHEMA_FILE_FIELD, state=DISABLED)
        self.field_scheme_export_dump_file.grid(column=5, row=start_row_position + 1, sticky='WE')
        self.field_scheme_export_dump_file.bind('<Double-Button-1>', self.choice_file)

        self.label_1 = Label(self.window)
        self.label_1.grid(column=6, row=start_row_position + 1, sticky='W')
        self.label_2 = Label(self.window)
        self.label_2.grid(column=7, row=start_row_position + 1, sticky='W')
        self.label_3 = Label(self.window)
        self.label_3.grid(column=8, row=start_row_position + 1, sticky='W')

    def choice_file(self, event):
        file = filedialog.asksaveasfilename(filetypes=(('Dump files', '*.dmp'), ('All files', '*.*')))
        self.scheme_name_export_dump_file.set(file)
        self.field_scheme_export_dump_file.configure(textvariable=self.scheme_name_export_dump_file)


# Основной класс отображения всех элементов окна для выполнения импорта в СУБД
class OracleExport(Frame):
    def __init__(self, parent, loop, config_file):
        ...
        self.button_export_scheme = Button(self.window,
                                           ...
                                           state=DISABLED)
        self.button_export_scheme.grid(column=3, columnspan=4, pady=18, row=self.shift_row_position + 1, sticky='WE')
    ...
    def set_visibility_scheme_gui_suits(self):
        for item in self.scheme_gui_suits:
            ...
            item.label_scheme_export_dump_file.config(state=current_state)
            item.field_scheme_export_dump_file.config(state=current_state)
    ...
    def export_schemes(self):
        ...
        for item in self.scheme_gui_suits:
            if item.checked.get() == int(item.scheme_id):
                scheme_name = item.field_scheme_name.get()
                scheme_password = item.field_scheme_password.get()
                scheme_export_dump_file = item.field_scheme_export_dump_file.get()
                oracle_string, oracle_string_mask_password = get_string_export_oracle_scheme(connection_string,
                                                                                             scheme_name,
                                                                                             scheme_password,
                                                                                             scheme_export_dump_file)
                self.loop.create_task(self.run_async_cmd_with_check_and_run_next_functions(oracle_string,
                                                                                           oracle_string_mask_password,
                                                                                           check_failure_result_export_oracle_scheme,
    ...
    async def run_async_cmd_with_check_and_run_next_functions(self,
        ...
        if process.returncode == 0 and self.oracle_execute_state:
            ...
            if run_next_function:
                run_next_function()
        # Данный случай для команды imp.exe
        elif process.returncode == 3 and self.oracle_execute_state:
    ...
    def _load_scheme_gui_suits(self, start_row_position):
        ...
        try:
            for scheme_id in sorted(self.config['scheme'].keys()):
                ...
                # real_file_name = get_real_file_name(scheme['export_dump_file'], scheme['enabled'])
    ...
    def _save_current_config(self):
        ...
        for item in self.scheme_gui_suits:
            ...
            if enabled:
                ...
                #self.config['scheme'][scheme_id]['export_dump_file'] = get_real_file_name(
                    # item.scheme_name_export_dump_file.get())
