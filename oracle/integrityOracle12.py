...
def get_string_make_pdb_writable(sysdba_name, sysdba_password, connection_string, pdb_name):
    """
        sqlplus c##devop/123devop@ORCL
        set echo on;
        set serveroutput on size unlimited;
        execute pdb.make_read_write('ASDCOMIK_88200');
    """
...
def check_failure_result_make_pdb_writable(log_string):
    log_string = log_string.upper()
    return log_string.startswith(CHECK_ORACLE_ERROR_0) or log_string.startswith(CHECK_ORACLE_ERROR_1)


def get_string_clone_pdb(sysdba_name, sysdba_password, connection_string, pdb_name, pdb_name_cloned):
    """
        sqlplus c##devop/123devop@ORCL
        set echo on;
        set serveroutput on size unlimited;
        execute pdb.clone_pdb('ASDCOMIK_88200', 'ASDCOMIK_88200_TEST');
    """
    script = f"""set echo on;
set serveroutput on size unlimited;
...
def check_failure_result_clone_pdb(log_string):
    log_string = log_string.upper()
    return log_string.startswith(CHECK_ORACLE_ERROR_0) or log_string.startswith(CHECK_ORACLE_ERROR_1)


def get_string_delete_pdb(sysdba_name, sysdba_password, connection_string, pdb_name):
    """
        sqlplus c##devop/123devop@ORCL
        set echo on;
        set serveroutput on size unlimited;
        execute pdb.remove('ASDCOMIK_88200_TEST');
    """
    script = f"""set echo on;
set serveroutput on size unlimited;
...
def check_failure_result_delete_pdb(log_string):
    log_string = log_string.upper()
    return log_string.startswith(CHECK_ORACLE_ERROR_0) or log_string.startswith(CHECK_ORACLE_ERROR_1)
