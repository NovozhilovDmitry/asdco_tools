...
watch_max_curs = """set linesize 200
column nv_isolat3 format a10
column d_exchangedate format a15
column f_rate format 99999.99999
set pagesize 1000
column d_exchangedate format a15
set feedback off
select c.nv_isolat3, e.d_exchangedate, e.f_rate from currency_hist c join exchangerates e on c.idcurrency=e.idcurrency where e.d_exchangedate = (select MAX(d_exchangedate) from exchangerates) and getoperdate between c.d_from and c.d_to order by c.idcurrency;"""
