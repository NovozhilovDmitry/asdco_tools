...
watch_max_curs = "select * from exchangerates where d_exchangedate in (select MAX(d_exchangedate) from exchangerates);"
...
is
    ...
    begin
          BEGIN
                ...
                DBMS_OUTPUT.PUT_LINE('Удалена схема ' || l_userName );
          EXCEPTION
          WHEN OTHERS
          THEN
             DBMS_OUTPUT.PUT_LINE('Получено исключение при удалении схемы ' || l_userName || ' : ' );
             DBMS_OUTPUT.PUT_LINE(dbms_utility.format_error_stack||dbms_utility.format_error_backtrace);
          END;
          
          l_sql:='CREATE USER ' || l_userName || ' IDENTIFIED BY '|| l_password;
          EXECUTE IMMEDIATE l_sql;
          DBMS_OUTPUT.PUT_LINE('Создана схема ' || l_userName );
          ...
          EXECUTE IMMEDIATE l_sql;
          DBMS_OUTPUT.PUT_LINE('Созданы grant ');
    end recreateUser;
    ...
    begin
          l_sql:=l_textSql;
          EXECUTE IMMEDIATE l_sql;
          DBMS_OUTPUT.PUT_LINE('Создана таблица ' || l_tableName);
    end createTable;
    ...
    begin
          ...
          EXECUTE IMMEDIATE l_sql;
          DBMS_OUTPUT.PUT_LINE('Создан grant select on ' || l_cxemaNSIinEXD || '.' || l_tableName || ' to ' || l_cxemaTUZinEXD );
...
begin
     ...
     DBMS_OUTPUT.PUT_LINE('Успешное завершение скрипта!');
     commit work;
end;
/
...
sql_dict = {
    ...
    'Установить подключение в настройках': add_exit_to_script(nsi_systemparameter),
    'Убрать график смен': add_exit_to_script(remove_schedule_for_users),
